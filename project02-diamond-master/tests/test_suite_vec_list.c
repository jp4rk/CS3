...
void test_list_size1() {
    ...
    *v = VEC_ZERO;
    ...
    vec_list_add(l, v);
    assert(vec_list_size(l) == 1);
    assert(vec_equal(*((vector_t *) vec_list_get(l, 0)), (vector_t) {1, 1}));
    // Modify
    *v = (vector_t) {1, 2};
    assert(vec_list_size(l) == 1);
    assert(vec_equal(*((vector_t *) vec_list_get(l, 0)), (vector_t) {1, 2}));
    vec_list_free(l);
}
...
void test_list_small() {
    ...
    *v = VEC_ZERO;
    ...
    assert(vec_equal(*((vector_t *) vec_list_get(l, 0)), VEC_ZERO));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 1)), (vector_t) {1, 1}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 2)), (vector_t) {2, 2}));
    // Fill to capacity
    v = malloc(sizeof(*v));
    v->x = v->y = 3;
    vec_list_add(l, v);
    ...
    assert(vec_equal(*((vector_t *) vec_list_get(l, 3)), (vector_t) {3, 3}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 4)), (vector_t) {4, 4}));
    // Remove some
    v = vec_list_remove(l);
    ...
    vec_list_add(l, v);
    assert(vec_list_size(l) == 5);
    assert(vec_equal(*((vector_t *) vec_list_get(l, 0)), (vector_t) {0, 0}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 1)), (vector_t) {1, 1}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 2)), (vector_t) {2, 2}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 3)), (vector_t) {5, 5}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 4)), (vector_t) {6, 6}));
    // Overwrite added elements
    *((vector_t *) vec_list_get(l, 3)) = (vector_t) {7, 7};
    *((vector_t *) vec_list_get(l, 4)) = (vector_t) {8, 8};
    assert(vec_equal(*((vector_t *) vec_list_get(l, 0)), (vector_t) {0, 0}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 1)), (vector_t) {1, 1}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 2)), (vector_t) {2, 2}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 3)), (vector_t) {7, 7}));
    assert(vec_equal(*((vector_t *) vec_list_get(l, 4)), (vector_t) {8, 8}));
    vec_list_free(l);
}
...
void test_list_large_get_set() {
    ...
    // Check
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        assert(vec_equal(*((vector_t *) vec_list_get(l, i)), (vector_t) {i, i}));
    }
...
void test_list_large_add_remove() {
    ...
    // Check all
    for (size_t i = 0; i < LARGE_SIZE; i++) {
        assert(vec_equal(*((vector_t *) vec_list_get(l, i)), (vector_t) {i + 1, i + 1}));
    }
    vec_list_free(l);
}
...
void test_out_of_bounds_access() {
    ...
    for (size_t size = 0; size <= max_size; size++) {
        ...
        for (access->index = 0; access->index < size; access->index++) {
            ...
            *((vector_t *) vec_list_get(access->list, access->index)) = new_vector;
            assert(vec_equal(*((vector_t *) vec_list_get(access->list, access->index)), new_vector));
        }
