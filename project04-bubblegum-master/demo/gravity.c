...
const double ELASTICITY = .75;
const double ACCELERATION = 75;
const int FRAME_WIDTH = 100;
const double RADIUS = 8.0;
const double X_VELO = 10;
const double Y_VELO = -3.5;
 ...
 * than 2.
 */
body_t *create_star(int num_points, float r, float g, float b) {
    assert(num_points > 2);
    ...
    body_t *star = body_init(points, num_points, color);
...
void bounce_check(body_t *body, vector_t min) {
    ...
    for (size_t i = 0; i < list_size(shape); i++) {
        ...
        if (current->y <= min.y) {
            ...
            vector_t centroid = {.x = body_get_centroid(body).x, .y = min.y + RADIUS};
...
void import_star(scene_t *scene, int prongs) {
    ...
    float g = rand_float();
    float b = rand_float();
    body_t *star = create_star(prongs, r, g, b);
    vector_t initial_velo = {.x = X_VELO, .y = Y_VELO};
...
// free it.
void scene_boundary_check(scene_t *scene, vector_t max) {
    size_t j = 0;
...
void update_coordinates_and_velocity(body_t *current_star, vector_t min, double dt) {
    // Update coordinates
    body_set_centroid(current_star,
                      vec_add(body_get_centroid(current_star),
                              vec_multiply(dt, body_get_velocity(current_star))));
    ...
    // Rotate the star
    body_set_rotation(current_star, body_get_angle(current_star) + ROTATION_FACTOR);
}

int main() {
    // Stars are created at (0,0), make this the top left.
    vector_t max = {.x = FRAME_WIDTH * 2, .y = 0};
    vector_t min = {.x = 0, .y = -1 * FRAME_WIDTH};
    sdl_init(min, max);
    ...
    while (!sdl_is_done(scene)) {
        ...
        for (size_t i = 0; i < scene_bodies(scene); i++) {
            ...
            update_coordinates_and_velocity(current_star, min, dt);
