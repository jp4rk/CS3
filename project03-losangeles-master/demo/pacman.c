...
body_t * create_pacman(rgb_color_t color) {
    list_t *points = list_init((5 * NUM_DEGREES) / 6 + 1, free);
    for (size_t i = 0; i < (5 * NUM_DEGREES) / 6; i++)  {
        vector_t new_pt = {
            .x = PACMAN_RADIUS,
            .y = 0
        };
...
vector_t random_centroid(size_t radius) {
    vector_t result = {
        .x = rand_float() * (FRAME_WIDTH - radius),
        .y = rand_float() * ((FRAME_WIDTH / 2) - radius)
    };
    return result;
}
...
void boundary_check(body_t *body) {
      ...
      for (size_t i = 0; i < list_size(shape); i++) {
          ...
          if (current->x < 0) {
              ...
              vector_t new_centroid = {
                  .x = centroid.x,
                  .y = (FRAME_WIDTH / 2) - PACMAN_RADIUS
              };
              body_set_centroid(body, new_centroid);
              break;
          }
          else if (current->y > FRAME_WIDTH / 2) {
